...
export const ContextCommand = {
     ...
     * @type {String}
     */
    description: "Comfort the User with a gentle headpat",
    ...
    async executeCommand(interaction, interactionUser) {
        ...
        if ( interaction.data.target_id === SenderId ) {
            displayMessage = localize(interaction.locale, 'ACTION_COMMAND_SELF_USER_HEADPAT', SenderDisplayName);
        }
        // Used on this App
        else if ( interaction.data.target_id === DISCORD_APP_USER_ID ) {
            displayMessage = localize(interaction.locale, 'ACTION_COMMAND_TWILITE_HEADPAT', SenderDisplayName);
        }
        // Used on Mee6, that yucky App that should be unverified :c
        else if ( interaction.data.target_id === "159985870458322944" ) {
            displayMessage = localize(interaction.locale, 'ACTION_COMMAND_MEE6_HEADPAT', SenderDisplayName, `<@159985870458322944>`);
        }
        // Used on any other App
        else if ( TargetUser.bot || TargetMember.user.bot ) {
            displayMessage = localize(interaction.locale, 'ACTION_COMMAND_OTHER_APPS_HEADPAT', SenderDisplayName, TargetDisplayName)
        }
        // Used on any non-App User
        else {
            displayMessage = localize(interaction.locale, 'ACTION_COMMAND_OTHER_USER_HEADPAT', SenderDisplayName, TargetDisplayName);
        }
