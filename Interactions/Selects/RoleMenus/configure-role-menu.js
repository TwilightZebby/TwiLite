...
import { ManageMessageEndpoint, RoleMentionRegEx } from '../../../Utility/utilityConstants.js';
...
export const Select = {
     ...
     * @type {String}
     */
    description: "Handles processing options during configuration of an existing Role Menu",
            ...
            case "cancel":
                return new JsonResponse({
                    ...
                    data: {
                        ...
                        components: [{ "type": ComponentType.TextDisplay, "content": localize(interaction.locale, 'ROLE_MENU_CONFIGURATION_CANCELLED') }]
                    }
                });
        }
    }
...
async function saveAndDisplay(interaction) {
    // Grab components to update Role Menu
    const MessageComponents = interaction.message.components;
    const MenuContainer = MessageComponents.find(comp => comp.type === ComponentType.Container);
    // Also grab message ID of the message to update this Menu into
    const MenuMessageId = interaction.data.custom_id.split("_").pop();


    // Post Menu
    let attemptMenuPost = await fetch(ManageMessageEndpoint(interaction.channel.id, MenuMessageId), {
        method: 'PATCH',
        headers: {
    ...
    if ( attemptMenuPost.status != 204 && attemptMenuPost.status != 200 ) {
        return new JsonResponse({
            ...
            data: {
                ...
                "content": localize(interaction.locale, 'ROLE_MENU_CONFIGURATION_ERROR_GENERIC')
            }
        });
    }
